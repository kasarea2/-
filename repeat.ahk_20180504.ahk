...
Weight_execute(){
	...
	;~ 아이템 창 드래그 후에 아이템 모양이 달라짐. 다시 클릭해서 방지
	sleep,1000
	...
	sleep,100
	;~ 잘못된 클릭으로 흑정령 들어가면 나가기
	if(search_img_GPS("Image\repeat\나가기.bmp",processId,x,y,70,나가기X1,나가기Y1,나가기X2,나가기Y2)){
		leftClick(x,y,1)
		return 
	}
	...
	if(horseOpen == "Y"){
		...
		loop,10
		{
			if(search_img_GPS("Image\상점.bmp",processId,x,y,120,행동X1,행동Y1,행동X2,행동Y2)){
				leftClick(x,y,1)
				sleep,1500
				break
			}else if(search_img_GPS("Image\상점2.bmp",processId,x,y,120,행동X1,행동Y1,행동X2,행동Y2)){
				leftClick(x,y,1)
				sleep,1500
				break
			}else if(search_img_GPS("Image\상점3.bmp",processId,x,y,120,상점_2X1,상점_2Y1,상점_2X2,상점_2Y2)){
				leftClick(x,y,1)
				sleep,1500
				break
			}	
...
petEatFurchase(){
	...
	sleep,1000
	;~ 펫상인에게 이동
	if(search_img_GPS("Image\repeat\펫상인마크.bmp",processId,x,y,50,상인리스트X1,상인리스트Y1,상인리스트X2,상인리스트Y2)){
		leftClick(x,y,1)
	}else{
		leftClick(320,165,1)
	}
	...
	sleep,23000
	;~ 펫상인클릭
	if(search_img_GPS("Image\repeat\펫상인.bmp",processId,x,y,85,행동X1,행동Y1,행동X2,행동Y2)){
		leftClick(x,y,1)
	}else{
		;~ 캐릭변경 후 사냥 돌입
		crtChangeRepeat()
	...
	;~ 즉시이동, 맥스, 확인 클릭, 종료
	sleep,1500
	if(search_img_GPS("Image\repeat\펫사료.bmp",processId,x,y,90,말아이템창X1,말아이템창Y1,말아이템창X2,말아이템창Y2)){
